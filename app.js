...
function displayDashboard() {
    ...
    if (verdictEl) {
        if (journeys.length === 0) {
            ...
            // UPDATED: Show correct break-even calculation
            if (verdictSubEl) verdictSubEl.textContent = `Zone 1–2 Annual Travelcard: £${ANNUAL_PASS.toLocaleString()}/yr ÷ ${WORKING_DAYS} days = ${formatCurrency(DAILY_BREAK_EVEN)}/day`;
            ...
            // UPDATED: Show correct break-even in stats
            document.getElementById('statPassDays').textContent = `${stats.calendarDays} calendar days × ${formatCurrency(DAILY_BREAK_EVEN)}`;
        ...
        return `
            <div class="analysis-row ${cls}">
                ...
                <div class="analysis-right">
                    ...
                    <div class="analysis-vs">vs ${formatCurrency(DAILY_BREAK_EVEN)} pass</div>
                    ${capNote}
                </div>
            </div>
        `;
...
function renderChart(summaries) {
    ...
    const options = {
        ...
        annotations: {
            yaxis: [{
                y: DAILY_BREAK_EVEN, // UPDATED: Now shows £6.88 reference line
                borderColor: '#d97706',
                borderWidth: 2,
                strokeDashArray: 6,
                label: {
                    text: `Break-even: ${formatCurrency(DAILY_BREAK_EVEN)}/day`, // UPDATED: Shows £6.88
                    position: 'front',
                    offsetX: 0,
                    style: {
                        color: '#fff',
